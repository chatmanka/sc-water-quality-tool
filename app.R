...
#           three QC columns (FalseLatitude, FalseLongitude, flag_negLat),
#           and an optional coordinate-error flag report formatted for
#           submission to WQX@epa.gov.
#
...
server <- function(input, output, session) {
    ...
    tagList(
      ...
      if (n_flags > 0) tagList(
        downloadButton("download_flags", 
                       ...
                       style = "width:100%; background:#e67e22 !important; border-color:#d35400 !important;"),
  ...
  output$preview_table <- DT::renderDataTable({
    ...
    preview <- head(df, 200)
    # Hide the flag column from the preview when no previewed rows are
    # flagged — reduces visual noise (the column is always in the CSV)
